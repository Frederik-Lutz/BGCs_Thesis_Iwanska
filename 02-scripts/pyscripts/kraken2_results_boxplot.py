...
# Filter data for the top 10 genera
top_10_genera_data = all_genera[all_genera['scientific_name'].isin(top_genera_names)]

# Sort genera by 'percentage' in descending order
sorted_genera = top_genera_overall.index.tolist()

# Create a boxplot
plt.figure(figsize=(12, 6))

boxplot_data = [all_genera_norm[all_genera_norm['scientific_name'] == genus]['percentage'] for genus in sorted_genera]

plt.boxplot(boxplot_data, vert=False)  # Change to horizontal boxplot
plt.xlabel('relative abundance')
plt.ylabel('genus')
plt.yticks(range(1, len(sorted_genera) + 1), sorted_genera)  # Set y-axis ticks to display genus names
plt.gca().set_yticklabels(['$\it{' + genus + '}$' for genus in sorted_genera])  # Set genus names in italic font
plt.show()
