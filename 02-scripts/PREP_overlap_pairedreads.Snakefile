...
rule join_json:
    input:
        expand("04-analysis/fastp_overlap/{sample}.merged.fastq.gz", sample=SAMPLES)
    output:
        "05-results/QUAL_DNA_molecule_length_dist.tsv"
    ...
    run:
        results = []
        for fn in input:
            with open(fn.replace(".merged.fastq.gz", ".fastp.json"), "rt") as jsonfile:
                ...
                results.append(pd.DataFrame.from_dict({'length': list(range(len(insertsize['histogram']))),
                            ...
                            .assign(sample=os.path.basename(fn).replace(".merged.fastq.gz", "")))
